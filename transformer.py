...
def greedy_decode(model, src, src_features, src_mask, article_features, max_len, start_symbol):
    ...
    for i in range(max_len-1):
        ...
        ys_features = torch.tensor([article_features[i] for i in ys], dtype=torch.double).to(DEVICE)
        # ys_features = torch.cat([ys_features, article_features[torch.unsqueeze(next_word.cpu(), 0)]], dim=0)
    return ys
...
#  src_features = [article_features[i] for i in src]
    for step, batch in tqdm(enumerate(batch_generator(indices, batch_size)), total=total_batches):
            ...
            tgt_ids = greedy_decode(
                model,  src, src_features, src_mask, article_features,  MAX_SEQUENCE_LENGTH, start_symbol=BOS_IDX)
